...
class RepViT(nn.Module):
    def __init__(self, cfgs, num_classes=1000, distillation=False):
        ...
        patch_embed = torch.nn.Sequential(
            QHWTD(3, input_channel // 2, 3, 3, 5),
            torch.nn.GELU(),
            # Conv2d_BN(input_channel // 2, input_channel, 3, 2, 1)
            QHWTD(input_channel // 2, input_channel, 3, 3, 4),
        )
        layers = [patch_embed]
...
def repvit_m0_6(num_classes=100):
    """
    ...
    return RepViT(cfgs, num_classes=num_classes, distillation=False)


def repvit_m0_9(num_classes=100):
    """
    Constructs a MobileNetV3-Large model
    """
    cfgs = [
