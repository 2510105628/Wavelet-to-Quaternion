...
class VSSM(nn.Module):
    ...
    def module_flops(self, shape=(3, 224, 224)):
        supported_ops = {
            "aten::silu": None,  # as relu is in _IGNORED_OPS
            "aten::neg": None,  # as relu is in _IGNORED_OPS
            "aten::exp": None,  # as relu is in _IGNORED_OPS
        ...
        model = copy.deepcopy(self)
        model.cuda().eval()
        dummy_input = torch.randn((1, *shape), device=next(model.parameters()).device)

        def forward_hook(module, input, output):
            module_inputs[module] = input[0]
        ...
        module_flops = {}
        for i, (module, input) in enumerate(module_inputs.items()):
            Gflops, unsupported = flop_count(module, (input,), supported_ops=supported_ops)
        ...
        del module_inputs, model, dummy_input
