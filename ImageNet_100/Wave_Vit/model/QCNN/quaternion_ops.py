...
def quaternion_dwconv(input, r_weight, i_weight, j_weight, k_weight, bias, stride,
                      padding, groups, dilatation):
    """
    Applies a quaternion convolution to the incoming data:
    """
    batch_size, in_channels, height, width = input.shape
    assert in_channels % 4 == 0, "Input channels must be divisible by 4 for quaternion convolution."
    groups //= 4
    split_size = in_channels // 4
    r_input = input[:, :split_size, :, :]
    i_input = input[:, split_size:2 * split_size, :, :]
    j_input = input[:, 2 * split_size:3 * split_size, :, :]
    k_input = input[:, 3 * split_size:, :, :]

    if input.dim() != 4:
        raise Exception("The convolutional input is 4 dimensions."
                        " input.dim = " + str(input.dim()))

    r_out = (F.conv2d(input=r_input, weight=r_weight, stride=(stride, stride), padding=(padding, padding),
                      groups=groups) + F.conv2d(input=i_input, weight=-i_weight, stride=(stride, stride),
                                                padding=(padding, padding), groups=groups) +
             F.conv2d(input=j_input, weight=-j_weight, stride=(stride, stride), padding=(padding, padding),
                      groups=groups) + F.conv2d(input=k_input, weight=-k_weight, stride=(stride, stride),
                                                padding=(padding, padding), groups=groups))
    i_out = (F.conv2d(input=r_input, weight=i_weight, stride=(stride, stride), padding=(padding, padding),
                      groups=groups) + F.conv2d(input=i_input, weight=r_weight, stride=(stride, stride),
                                                padding=(padding, padding), groups=groups) +
             F.conv2d(input=j_input, weight=-k_weight, stride=(stride, stride), padding=(padding, padding),
                      groups=groups) + F.conv2d(input=k_input, weight=j_weight, stride=(stride, stride),
                                                padding=(padding, padding), groups=groups))
    j_out = (F.conv2d(input=r_input, weight=j_weight, stride=(stride, stride), padding=(padding, padding),
                      groups=groups) + F.conv2d(input=i_input, weight=k_weight, stride=(stride, stride),
                                                padding=(padding, padding), groups=groups) +
             F.conv2d(input=j_input, weight=r_weight, stride=(stride, stride), padding=(padding, padding),
                      groups=groups) + F.conv2d(input=k_input, weight=-i_weight, stride=(stride, stride),
                                                padding=(padding, padding), groups=groups))
    k_out = (F.conv2d(input=r_input, weight=k_weight, stride=(stride, stride), padding=(padding, padding),
                      groups=groups) + F.conv2d(input=i_input, weight=-j_weight, stride=(stride, stride),
                                                padding=(padding, padding), groups=groups) +
             F.conv2d(input=j_input, weight=i_weight, stride=(stride, stride), padding=(padding, padding),
                      groups=groups) + F.conv2d(input=k_input, weight=r_weight, stride=(stride, stride),
                                                padding=(padding, padding), groups=groups))
