...
class QHWTD(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, wt_kernel_size=3, wt_levels=2, wt_type='db1'):
        ...
        self.wavelet_convs = nn.ModuleList(
            ...
            [QuaternionConv(in_channels, in_channels, wt_kernel_size, 1, (kernel_size - 1) // 2, bias=True) for _ in
             range(self.wt_levels)]
        )
        self.wavelet_scale = nn.ModuleList(
            [_ScaleModule([1, in_channels, 1, 1], init_scale=0.1) for _ in range(self.wt_levels)]
            # [nn.Sequential(nn.BatchNorm2d(in_channels),nn.ReLU(inplace=True)) for _ in range(self.wt_levels)]
        )
