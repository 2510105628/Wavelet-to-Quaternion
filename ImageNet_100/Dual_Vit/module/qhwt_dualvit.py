...
import math
import numpy as np
from model.DWT.QHWTD import QHWTD
...
class DualAttention(nn.Module):
    ...
    def forward(self, x, H, W, semantics):
        ...
        q = self.q(x).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        q_semantics = self.q_proxy(self.q_proxy_ln(semantics)).reshape(B_p, N_p, self.num_heads,
        ...
        kv = self.kv(self.proxy_ln(semantics)).reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3,
                                                                                                              1, 4)
        k, v = kv[0], kv[1]
...
class DownSamples(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        # 原：self.proj = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=2, padding=1)
        self.proj = QHWTD(in_channels, out_channels, 3, 3, levels.pop())
...
class Stem(nn.Module):
    def __init__(self, in_channels, stem_hidden_dim, out_channels):
        ...
        self.proj = nn.Sequential(
            nn.Conv2d(hidden_dim,out_channels,3,2,1)
        )
        self.norm = nn.LayerNorm(out_channels)
        ...
        for i in range(self.sep_stage):
            ...
            if i == 0:
                ...
                if x_down.shape[1] == self.q.shape[0]:
                    ...
                    self_q = F.interpolate(self_q.unsqueeze(0), size=(x_down_H, x_down_W), mode='bicubic').squeeze(
                        0).permute(1, 2, 0)
