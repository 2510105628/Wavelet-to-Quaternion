...
def qconv_1x1(inp, oup):
    return QuaternionConv(inp, oup, 1, 1, 0, bias=False)
...
#
class QHWTD_V2(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, wt_kernel_size=3, wt_levels=2, wt_type='db1'):
        super(QHWTD_V2, self).__init__()

        self.wt_levels = wt_levels
        self.wt_filter, self.iwt_filter = wavelet.create_wavelet_filter(wt_type, in_channels // 4, in_channels // 4,
                                                                        torch.float)
        ...
        self.wavelet_convs = nn.ModuleList(
            ...
            [
                QuaternionConv(in_channels, in_channels, wt_kernel_size, 1, (wt_kernel_size - 1) // 2, bias=False)
                for _ in range(self.wt_levels)
            ]
        )
        self.wavelet_scale = nn.ModuleList(
            # [_ScaleModule([1, in_channels, 1, 1], init_scale=0.1) for _ in range(self.wt_levels)]
            [nn.BatchNorm2d(in_channels) for _ in range(self.wt_levels)]
        )
        ...
        self.downSample = nn.Sequential(
            nn.Conv2d(in_channels + in_channels // 4, out_channels, kernel_size, 2, (kernel_size-1)//2, bias=False),
        )
...
class QHWTD_V3(nn.Module):
    def __init__(self, in_channels, out_channels, wt_kernel_size=3, wt_levels=2, bn='scale',wt_type='db1'):
        super(QHWTD_V3, self).__init__()
        ...
        if bn == 'scale':
            self.wavelet_convs = nn.ModuleList(
                [QuaternionConv(in_channels*4, in_channels*4, wt_kernel_size, 1, (wt_kernel_size - 1) // 2, bias=True)
                 for _ in range(self.wt_levels)]
            )
            self.wavelet_scale = nn.ModuleList(
            ...
            self.wavelet_scale = nn.ModuleList(
                [nn.Sequential(nn.BatchNorm2d(in_channels*4)) for _ in range(self.wt_levels)]
            )
