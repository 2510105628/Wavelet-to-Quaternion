...
from Wavelet_to_Quaternion.QHWT.DWT.QHWT import QHWT
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        if stride == 2:
            self.conv1 = nn.Sequential(
                QHWT(inplanes,3,3),
                conv3x3(inplanes+inplanes//4,planes,stride)
            )
        else:
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        if stride == 1:
            ...
            self.conv2 = nn.Sequential(
                QHWT(width,3,3),
                conv3x3(width+width//4,width,stride,groups,dilation)
            )
...
def resnext50_32x4d(**kwargs):
    r"""ResNeXt-50 32x4d module from
