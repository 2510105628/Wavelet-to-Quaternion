...
def load_checkpoint(config, model, optimizer, lr_scheduler, scaler, logger):
    ...
    for k in rpe_mlp_keys:
        checkpoint['module'][k.replace('rpe_mlp', 'cpb_mlp')] = checkpoint['module'].pop(k)
...
def save_checkpoint(config, epoch, model, max_accuracy, optimizer, lr_scheduler, scaler, logger):
    save_state = {'module': model.state_dict(),
...
def load_pretrained(config, model, logger):
    ...
    if any([True if 'encoder.' in k else False for k in checkpoint_model.keys()]):
        ...
        logger.info('Detect pre-trained module, remove [encoder.] prefix.')
    else:
        logger.info('Detect non-pre-trained module, pass without doing anything.')
