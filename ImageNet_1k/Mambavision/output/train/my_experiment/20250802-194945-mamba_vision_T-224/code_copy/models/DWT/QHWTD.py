...
class QHWTD_V2(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, wt_levels=2, wt_type='db1'):
        ...
        self.wavelet_convs = nn.ModuleList(
            ...
            #            bias=True) for _ in range(self.wt_levels)]
            [QuaternionConv(in_channels, in_channels, kernel_size, 1, (kernel_size - 1) // 2, bias=True) for _ in
        ...
        self.downSample = nn.Sequential(
            nn.Conv2d(in_channels + in_channels // 4, out_channels, 3, 2, 1, bias=False),
        )
    ...
    def forward(self, x):
        ...
        x = self.downSample(x)
        # res_qwt = self.base_conv(res)
        # res_pool = self.maxpool(x)
        # res = x + res_qwt
        return x
...
class QHWTD_V3(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, wt_kernel_size=3,wt_levels=2, wt_type='db1'):
        ...
        self.wavelet_convs = nn.ModuleList(
            ...
            [QuaternionConv(in_channels*4, in_channels*4, wt_kernel_size, 1, (wt_kernel_size - 1) // 2, bias=True) for _ in
             range(self.wt_levels)]
        )
        self.wavelet_scale = nn.ModuleList(
