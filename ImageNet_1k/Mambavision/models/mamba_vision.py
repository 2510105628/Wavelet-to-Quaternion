...
def _load_state_dict(module, state_dict, strict=False, logger=None):
    ...
    if len(err_msg) > 0:
        err_msg.insert(
            0, 'The module and loaded state dict do not match exactly\n')
        err_msg = '\n'.join(err_msg)
        if strict:
...
def _load_checkpoint(model,
    ...
    Args:
        ...
        strict (bool): Whether to allow different params for the module and
            checkpoint.
...
class Downsample(nn.Module):
    def __init__(self,
        ...
        self.reduction = nn.Sequential(
            # QHWT(in_channels=dim, kernel_size=3, wt_levels=3,scale='scale'),
            # nn.Conv2d(dim+dim//4,dim_out,3,2,1,bias=False),
