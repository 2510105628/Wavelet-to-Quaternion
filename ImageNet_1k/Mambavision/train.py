...
group.add_argument('--module', default='gc_vit_tiny', type=str, metavar='MODEL',
                    help='Name of module to train (default: "gc_vit_tiny"')
group.add_argument('--pretrained', action='store_true', default=False,
                    help='Start with pretrained version of specified network (if avail)')
group.add_argument('--initial-checkpoint', default='', type=str, metavar='PATH',
                    help='Initialize module from this checkpoint (default: none)')
group.add_argument('--resume', default='', type=str, metavar='PATH',
                    help='Resume full module and optimizer state from checkpoint (default: none)')
group.add_argument('--loadcheckpoint', default='', type=str, metavar='PATH',
                    help='Resume full module and optimizer state from checkpoint (default: none)')
group.add_argument('--no-resume-opt', action='store_true', default=False,
                    help='prevent resume of optimizer state when resuming module')
group.add_argument('--num-classes', type=int, default=None, metavar='N',
                    help='number of label classes (Model default if None)')
group.add_argument('--gp', default=None, type=str, metavar='POOL',
                    help='Global pool type, one of (fast, avg, max, avgmax, avgmaxc). Model default if None.')
group.add_argument('--img-size', type=int, default=None, metavar='N',
                    help='Image patch size (default: None => module default)')
group.add_argument('--input-size', default=None, nargs=3, type=int,
                    metavar='N N N', help='Input all image dimensions (d h w, e.g. --input-size 3 224 224), uses module default if empty')
...
group.add_argument('--interpolation', default='', type=str, metavar='NAME',
                    help='Image resize interpolation type (overrides module)')
...
scripting_group.add_argument('--torchscript', dest='torchscript', action='store_true',
                    help='torch.jit.script the full module')
scripting_group.add_argument('--aot-autograd', default=False, action='store_true',
                    help="Enable AOT Autograd support. (It's recommended to use this option with `--fuser nvfuser` together)")
group.add_argument('--fuser', default='', type=str,
                    help="Select jit fuser. One of ('', 'te', 'old', 'nvfuser')")
group.add_argument('--grad-checkpointing', action='store_true', default=False,
                    help='Enable gradient checkpointing through module blocks/stages')
...
group.add_argument('--module-ema', action='store_true', default=False,
                    help='Enable tracking moving average of module weights')
group.add_argument('--module-ema-force-cpu', action='store_true', default=False,
                    help='Force ema to be tracked on CPU, rank=0 node only. Disables EMA validation.')
group.add_argument('--module-ema-decay', type=float, default=0.9998,
                    help='decay factor for module weights moving average (default: 0.9998)')
...
group.add_argument('--validate_only', action='store_true', default=False,
                    help='run module validation only')
...
group.add_argument('--master-port', type=int, default=29501,
                   help='Master port for distributed training (default: 29501)')
...
def main():
    ...
    if args.num_classes is None:
        ...
        args.num_classes = model.num_classes  # FIXME handle module default vs config num_classes more elegantly
    ...
    # move module to GPU, enable channels last layout if set
    model.cuda()
    with torch.cuda.amp.autocast(enabled=True):
        flops_count, params_count = get_model_complexity_info(model, (3, 224, 224), as_strings=True,
                                                              print_per_layer_stat=False, verbose=False)
    print(f'Model %s created, flops_count: %s, param count: %s' % ('module', flops_count, params_count))
    ...
    if args.distributed and args.sync_bn:
        ...
        if args.local_rank == 0:
            _logger.info(
                'Converted module to use Synchronized BatchNorm. WARNING: You may have issues if using '
                'zero initialized BN layers (enabled by default for ResNets) while sync-bn enabled.')

    if args.torchscript:
        assert not use_amp == 'apex', 'Cannot use APEX AMP with torchscripted module'
        assert not args.sync_bn, 'Cannot use SyncBatchNorm with torchscripted module'
    ...
    # setup exponential moving average of module weights, SWA could be used here too
    model_ema = None
    if args.model_ema:
        # Important to create EMA module after cuda(), DP wrapper, and AMP but before SyncBN and DDP wrapper
    ...
    if args.distributed:
        ...
        # NOTE: EMA module does not need to be wrapped by DDP
